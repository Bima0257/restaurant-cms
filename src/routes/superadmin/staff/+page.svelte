<script lang="ts">
  ...
  onMount(async () => {
    try { staffList = await api.superadminListStaff(); }
   ...
   function resetForm() { form = { email: "", password: "", full_name: "", phone: "" }; editing = null; }

  function editStaff(staff: any) {
    form = { email: staff.email, password: "", full_name: staff.full_name, phone: staff.phone || "" };
    editing = staff; showForm = true;
  }

  async function save() {
    try {
      if (editing) {
        await api.superadminUpdateStaff(editing.id, { full_name: form.full_name, email: form.email, phone: form.phone || undefined });
        toast.success("Staff updated successfully");
      } else {
        await api.superadminCreateStaff(form);
        toast.success("Staff created successfully");
      }
      staffList = await api.superadminListStaff(); showForm = false; resetForm();
  ...
  async function deleteStaff(staff: any) {
    ...
    if (!confirmed) return;
    try { await api.superadminDeleteStaff(staff.id); staffList = staffList.filter((s) => s.id !== staff.id); toast.success("Staff deleted successfully"); }
    catch (err: any) { toast.error(err.message); }
  }
</script>

<div class="flex items-center justify-between mb-8">
  <h1 class="font-headline-h2 text-headline-h2 text-ivory-white">Staff Management</h1>
...
{#if showForm}
  <div class="bg-surface-card rounded-2xl border border-deep-border p-6 mb-8 max-w-lg">
    <h3 class="font-headline-h3 text-headline-h3 text-ivory-white mb-4">{editing ? "Edit" : "Create"} Staff</h3>
...
<div class="bg-surface-card rounded-2xl border border-deep-border p-6">
  <DataTable {columns} rows={staffList} {loading} onEdit={editStaff} onDelete={deleteStaff} />
</div>
