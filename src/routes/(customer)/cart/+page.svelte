...
<div class="pt-32 pb-20 px-gutter max-w-grid-max-width mx-auto">
  <div class="mb-8">
    <h1 class="font-headline-h1 text-headline-h1 flex items-center gap-3">
      <ShoppingCart size={32} class="text-flame-orange" />
      Your Cart
    </h1>
  </div>
  ...
  {:else if items.length === 0}
    <div class="text-center py-20">
      <ShoppingCart size={64} class="text-muted-gray mx-auto mb-4" />
      <p class="text-muted-gray text-lg mb-4">Your cart is empty</p>
  ...
  {:else}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2 space-y-4">
        {#each items as item}
          <div class="bg-surface-card rounded-2xl border border-deep-border p-4 flex items-center gap-4">
            <img src={item.menu_item_image} alt={item.menu_item_name} class="w-20 h-20 rounded-xl object-cover" />
            <div class="flex-1">
              <h4 class="font-headline-h3 text-headline-h3 text-ivory-white">{item.menu_item_name}</h4>
            ...
            <p class="text-ivory-white font-bold w-20 text-right">${Number(item.subtotal).toFixed(2)}</p>
            <button onclick={() => removeItem(item.id)} class="text-red-400 hover:text-red-300 transition-colors cursor-pointer">
              <Trash2 size={18} />
            </button>
          </div>
        {/each}
